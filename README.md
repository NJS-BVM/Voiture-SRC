...
On a aussi fait en sorte de régler la vitesse de la voiture suivant si on était à la position maximale du joystick ou non. L'application est disponible sur Android et IOS, avec des interfaces différentes. La version Android est un peu moins intuitive surtout pour l'arrêt de la voiture étant donné qu'on devait retrouver le milieu exact du joystick pour qu'elle s'arrête.
...
Pour effectuer le suivi de ligne, on a travaillé avec un mode qui permet de repérer par des vecteurs sous forme de flèches sur l'écran une ligne à suivre, et renvoie l'origine de la flèche. La flèche suit la moyenne des lignes du circuit c’est-à-dire s'il y a un angle droit la flèche ne suivra pas parfaitement cet angle mais tournera d'une manière plus arrondie. Dans le code, on a baissé la vitesse de la voiture pour donner plus de temps à la caméra pour créer la trajectoire. Pour le côté manuel on a dû incliner légèrement la caméra pour ne pas avoir de lignes parasites que la caméra aurait pu suivre, sans trop la baisser pour qu'elle puisse avoir assez de temps pour prévoir la trajectoire s'il y a des angles droits. 
...
IV/La dernière partie de notre projet a consisté à obtenir un code qui permet à la voiture de retrouver une balise. On a utilisé deux cartes Lora, un récepteur branché sur la voiture qui allume les LEDs suivant la puissance reçue et un émetteur. 
Pour effectuer cette partie on a décidé que la manière la plus simple était que la voiture tourne à gauche quand elle s'éloignait de l'emetteur et quand elle apercevait une augmentation de la valeur reçue elle avançait et accélérait légèrement plus. Quand l'émetteur est suffisamment proche du récepteur la voiture s'arrête grâce à un changement d'état dans le code.
Pour le code on récupère le niveau de puissance grâce au programme de la carte uca. Ce niveau de puissance est reçu en analogique, mais la carte ne peut pas analyser de l'analogique on a alors branché la carte sur 7 sorties digitales où on récupèrait la donnée et la traduisit en binaire pour l'envoyer sur chaque bit de la sortie reliée à l'entrée sur la carte de la voiture où on récupère et reconvertit la valeur pour pouvoir l'interpréter.

Cependant on a rencontré un problème étant donné que les cartes ne reçevaient pas les bonnes informations puisque le problème était lié au branchement sur la broche D13, qui était bloqué comme horloge et la broche D8, qui était bloqué comme reset. Étant donné qu'il n'y avait pas d'autres pins disponibles, on a dû enlever 2 câbles et ainsi on n'a plus qu'une précision de 32 bits et non 128.
